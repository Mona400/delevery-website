...
export class ProductComponent implements OnInit {
  ...
  deleteMeal(product){
    ...
    .subscribe({next: async (res)=>{
      await sweetAlert("Done" , `${product.title} has been deleted` , 'success')
  ...
  UpdateMeal(item:any){
    ...
    .subscribe({next: async (res)=>{ await sweetAlert("success",`${this.form2.value['title']} has been updated` , 'success')},
